...
async def update_servant(id : str,servant : UpdateServant | None = None,alamat_servant : UpdateAlamat | None = None,detail_servant : UpdateDetailservant | None = None, session : AsyncSession = None) -> ServantBase : 
    statementUser = await session.execute(select(User).options(joinedload(User.alamat),joinedload(User.servant).options(joinedload(Detail_Servant.pelayanan))).where(User.id == id))
    findUser = statementUser.scalars().first()
...
async def getAllServant(session : AsyncSession) -> list[ServantBase] :
    statement = await session.execute(select(User).options(joinedload(User.alamat),joinedload(User.servant).options(joinedload(Detail_Servant.pelayanan))))
...
async def searchServant(page : int,filter : SearchServant,session : AsyncSession) -> SearchServantResponse :
    searchFilterQuery = and_(User.username.like(f"%{filter.username}%") if filter.username else True,User.online == filter.online if filter.online else True,User.servant.has(ready_order=filter.ready_order) if filter.ready_order else True)
    # 
    ...
    countPage = math.ceil(statementCountPage.scalar_one())
...
async def getServantById(id : str,session : AsyncSession) -> MoreServantBase :
    ...
    statement = await session.execute(select(User,func.avg(Rating.rating).label("total rating")).select_from(User).join_from(User,Detail_Servant,User.id == Detail_Servant.id_servant,full=True).join_from(Detail_Servant,Rating,Rating.id_detail_servant == Detail_Servant.id,full=True).group_by(User,Detail_Servant).options(joinedload(User.alamat),moreDetailServantQueryOption).where(User.id == id))
