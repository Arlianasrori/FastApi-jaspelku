...
from ..domain.servant.servantModel import AddDetailServant,ResponseAddUpdateDetailServant,ResponseProfilServant,ResponseGetServant,UpdateProfileServant,ResponseDetailProfileServant,ResponseRatingsServant,ResponseGetRatingById,ResponseGetPesanans,ResponseGetPesananBYId
...
# profile
@servantRouter.post("/profile",response_model=ResponseModel[ResponseAddUpdateDetailServant],tags=["SERVANT/PROFILE"])
...
async def addDetailServant(user : dict = Depends(GetUserDepends),session : sessionDepedency = None) :
...
# pesanan
@servantRouter.get("/pesanan",response_model=ResponseModel[ResponseGetPesanans],tags=["SERVANT/PESANAN"])
async def addDetailServant(status_pesanan : Status_Pesanan_Enum | None = None,user : dict = Depends(GetUserDepends),session : sessionDepedency = None) :
...
async def addDetailServant(id_pesanan : str | None = None,session : sessionDepedency = None) :
...
async def addDetailServant(id_pesanan : str | None = None,user : dict = Depends(GetUserDepends),session : sessionDepedency = None) :
...
async def addDetailServant(approved : bool,id_pesanan : str | None = None,user : dict = Depends(GetUserDepends),session : sessionDepedency = None) :
